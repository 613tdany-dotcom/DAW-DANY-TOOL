...
(() => {
  ...
  class AudioTrack{
    constructor(name, buf){ this.kind='audio'; this.name=name; this.buf=buf; this.gain=0.9; this.pan=0; this.mute=false; this.solo=false; this.sendA=0; this.sendB=0; this.eq={ enabled:true, hpf:{freq:60,q:0.707}, peak:{freq:1000,gain:0,q:1}, lpf:{freq:12000,q:0.707} }; this.eqNodes=null; this.node=null; this.gainNode=null; this.panNode=null; this.busGain=null; this.sendAGain=null; this.sendBGain=null; this.meter=null; this.canvas=null; }
    ...
    stop(){ try{ this.node && this.node.stop(); }catch{} this.node=null; this.gainNode=null; this.panNode=null; this.busGain=null; this.sendAGain=null; this.sendBGain=null; this.eqNodes=null; this.meter=null; }
    getDuration(){ return this.buf? this.buf.duration : 0; }
  }
  class InstrumentTrack{
    constructor(name='Instrumento'){ this.kind='instrument'; this.name=name; this.notes=[]; this.gain=0.6; this.pan=0; this.mute=false; this.solo=false; this.sendA=0; this.sendB=0; this.eq={ enabled:true, hpf:{freq:60,q:0.707}, peak:{freq:1000,gain:0,q:1}, lpf:{freq:12000,q:0.707} }; this.gainNode=null; this.busGain=null; this.sendAGain=null; this.sendBGain=null; this.eqNodes=null; this.meter=null; this.canvas=null; }
  ...
  function renderMixer(){ const mixView=$('#mixerView'); const strips=$('#mixerStrips'); const bus=$('#busControls'); if(!mixView||!strips||!bus) return; bus.innerHTML = '';
    ...
    function upd(i, path, v){ const tr=S.tracks[i]; const seg=path.split('.'); if(seg[0]==='hpf'){ tr.eq.hpf[seg[1]]=v; if(tr.eqNodes?.hpf){ if(seg[1]==='freq') tr.eqNodes.hpf.frequency.value=v; else if(seg[1]==='q') tr.eqNodes.hpf.Q.value=v; } }
    ...
    strips.querySelectorAll('.eq-hpf-f').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'hpf.freq', +e.target.value)));
    strips.querySelectorAll('.eq-hpf-q').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'hpf.q', +e.target.value)));
    strips.querySelectorAll('.eq-peak-f').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'peak.freq', +e.target.value)));
    strips.querySelectorAll('.eq-peak-g').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'peak.gain', +e.target.value)));
    strips.querySelectorAll('.eq-peak-q').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'peak.q', +e.target.value)));
    strips.querySelectorAll('.eq-lpf-f').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'lpf.freq', +e.target.value)));
    strips.querySelectorAll('.eq-lpf-q').forEach(inp=> inp.addEventListener('input', e=> upd(+e.target.dataset.i, 'lpf.q', +e.target.value)));
